<?PHP
    ...
    class DBObject
    {
        ...
        public function select($id, $column = null)
        {
			...
			if(is_array($id)){ // Multiple ids can be passed optionally as an array
				
				$where = 'WHERE ';
				
				foreach($id as $key => $value):
				
					$where .= '`'.$column[$key].'` = \'?\' AND ';
				
				endforeach;
				
	            $db->query(str_replace('AND LIMIT','LIMIT','SELECT * FROM `'.$this->tableName.'` '.$where.'LIMIT 1'), $id);
        ...
        public static function glob($class_name, $sql = null)
        {
            ...
            $sql_query = 'SELECT * FROM `'.$tmp_obj->tableName.'`';

            if(!is_null($sql))
                $sql_query .= $sql;

            $objs = array();
            $rows = $db->getRows($sql_query);
            foreach($rows as $row)
            {
                $o = new $class_name;
                $o->load($row);
