<?php
    // Orm_Object is brand new and totally untested as of 04/02/2009 - RTH
    class Orm_Object extends Db_Object
    {
        public static $_data;

        public function __construct($table_name, $columns, $id = null)
        {
            if (!is_array(Orm_Object::$_data))
                Orm_Object::$_data = array();
        ...
        public function __get($key)
        {
            ...
            if (isset(Orm_Object::$_data[$this->className][strtolower($key)]))
                return $this->{Orm_Object::$_data[$this->className][strtolower($key)]['getter']}($key);

            $trace = debug_backtrace();
            trigger_error("Undefined property via Orm_Object::__get(): $key in {$trace[0]['file']} on line {$trace[0]['line']}", E_USER_NOTICE);
            return null;
        }
        ...
        public function __set($key, $value)
        {
            ...
            elseif (isset(Orm_Object::$_data[$this->className][strtolower($key)]))
                return $this->{Orm_Object::$_data[$this->className][strtolower($key)]['setter']}($key, $value);
        ...
        {
            if (isset(Orm_Object::$_data[$this->className][strtolower($name)]))
                return $this->{Orm_Object::$_data[$this->className][strtolower($name)]['func']}($name, $arguments);
        }
        ...
        public function belongsTo($class_name, $pk = null, $fk = null)
        {
            ...
            if (@!is_array(Orm_Object::$_data[$this->className])) Orm_Object::$_data[$this->className] = array();
            Orm_Object::$_data[$this->className][strtolower($class_name)] =
                    array('pk' => $pk,
                          'fk' => $fk,
                          'pc' => $class_name,
                          'fc' => $this->className,
        ...
        {
            if (!is_subclass_of($val, 'Db_Object'))
            {
                trigger_error("Cannont assign non-Db_Object to Orm_Object property $key in {$trace[0]['file']} on line {$trace[0]['line']}", E_USER_NOTICE);
                return;
            }
        ...
        public function hasMany($class_name, $pk = null, $fk = null, $joined = false)
        {
            ...
            if (is_null($fk)) $fk = strtolower($this->className . '_id');
            if (@!is_array(Orm_Object::$_data[$this->className])) Orm_Object::$_data[$this->className] = array();
            Orm_Object::$_data[$this->className][strtolower($class_name . 's')] =
                    array('pk' => $pk,
                          'fk' => $fk,
                          'pc' => $this->className,
                          'fc' => $class_name,
            ...
            Orm_Object::$_data[$this->className][strtolower($class_name . 'Ids')] =
                    array('pk' => $pk,
                          'fk' => $fk,
                          'pc' => $this->className,
                          'fc' => $class_name,
            ...
            Orm_Object::$_data[$this->className][strtolower('num' . ucfirst($class_name) . 's')] =
                    array('pk' => $pk,
                          'fk' => $fk,
                          'pc' => $this->className,
                          'fc' => $class_name,
            ...
            Orm_Object::$_data[$this->className][strtolower('clear' . ucfirst($class_name) . 's')] =
                    array('pk' => $pk,
                          'fk' => $fk,
                          'pc' => $this->className,
                          'fc' => $class_name,
